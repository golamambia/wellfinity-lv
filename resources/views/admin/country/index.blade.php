...
<div class="content-wrapper">
  ...
  @endif
   ...
   <div class="alert alert-success alert-dismissible">
    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
    <h4><i class="icon fa fa-ban"></i> Success</h4>
    Country Status is updated successfully.
  </div>
  @endif
        ...
        <table id="example2" class="table table-bordered table-hover dataTable">
          ...
          @if ($lists->count() > 0)
           ...
           <tr>
            @foreach($column_array as $key => $value)
              ...
              @elseif ($key=='image')              
                <td>
                  ...
                  <img src="{{ asset('/uploads/'.$list->image) }}" style="max-width: 200px;">
                  @endif
                </td>
              @elseif ($key=='status')              
            ...
            <td>
              <a href="{{ url('/admin/country/edit/'.$list->id) }}" title="Edit"><i class="fa fa-fw fa-edit"></i></a>
              <a href="{{ url('/admin/country/status/'.$list->id.'/'.$list->status) }}" title="Status"><i class="fa fa-fw fa-lightbulb-o {{$list->status!=1?'inactive':''}}"></i></a>
              <a href="{{ url('/admin/country/delete/'.$list->id) }}" onclick="return confirm('Are you sure?');" title="Delete"><i class="fa fa-fw fa-close"></i></a>
            </td>
